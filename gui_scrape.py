...
from seo_scraper import fetch_title, fetch_links
...
if __name__ == "__main__": # execute logic if run directly
    ...
    # Button for fetching the page title
    _fetch_title_btn = ttk.Button(
        _url_frame, text='Fetch title', command=lambda: sb(fetch_title(_url.get())))
    _fetch_title_btn.grid(row=1, column=0, sticky=W, padx=5)

    # Button for fetching external links
    _fetch_links_btn = ttk.Button(
    _url_frame, text = 'Fetch links', command = lambda: sb('\n'.join(fetch_links(_url.get()))))
    _fetch_links_btn.grid(row=1, column=1, sticky=W, padx=5)
